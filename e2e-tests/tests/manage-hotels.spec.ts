...
test("should allow user to add a hotel", async ({ page }) => {
  ...
  await expect(page.getByText("Hôtel sauvegardé !")).toBeVisible()
})
...
test("should display hotels", async ({ page }) => {
  ...
  await expect(page.getByText("2 adults, 3 childrens")).toBeVisible()
...
test("should edit hotel", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Sauvegarder" }).click()
  await expect(page.getByText("Hotel sauvegardé !")).toBeVisible()
